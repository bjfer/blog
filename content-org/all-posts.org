...
For the last couple of years, I have been dealing with [[https://www.orthopaediezentrum-muenchenost.de/en/orthopedics/diseases/carpal-tunnel-syndrome/][carpal tunnel syndrome]] (CTS). My first instinct was to blame my heavy keyboard use (and, by proxy, my Emacs-Stockholm syndrome). Some studies seem to support that idea, especially for mouse users, while others suggest [[https://www.sciencedirect.com/science/article/abs/pii/S0022510X14008028][a link]] is weak or [[https://onlinelibrary.wiley.com/doi/epdf/10.1002/art.22956][unlikely altogether]]. In any case, while looking for ways to reduce the problem, I ended up going down the rabbit hole of split keyboards.

There is no shortage of information online about the benefits of split keyboards. I had been vaguely aware of them before, but never considered buying one. What kept coming up in online communities was how much they had helped people with CTS, so I decided to try one myself. There are many companies making these keyboards. Some offer DIY kits, allowing one to solder your own and customize nearly everything: switches, keycaps, colors, RGB lighting, and more. Beyond the hardware, there are also open-source firmware options, various configuration tools, and countless possibilities for remapping keys. It can all be a bit overwhelming. In this post and the ones that follow, I will try to summarize my experience with three different split keyboards.
...
* DONE Quick Git tip                                                    :git:
CLOSED: [2025-10-20 Mon 11:34]
:PROPERTIES:
:EXPORT_FILE_NAME: git-tip
:hidedoc:  true
:END:
...
* DONE Dotfiles with Org Babel and Host-Specific Configuration    :emacs:org:
CLOSED: [2026-08-14 Fri 00:37]
:PROPERTIES:
:EXPORT_FILE_NAME: dotfile-org
:hidedoc:  true
:END:

Like many Emacs users, I use ~org-babel-load-file~ to load custom Emacs Lisp source blocks from an Org file ([[https://orgmode.org/worg/org-contrib/babel/intro.html#emacs-initialization][as described here]]). I also use Org Babel’s tangling support to maintain separate Org files for several dotfiles, such as =~/.bashrc=, =~/.gitconfig=, =~/.signature=, and =~/.ssh/config=. This helps me keep my setup consistent across work and home machines.
...
For static information, I define named blocks and reference them using Org’s [[https://orgmode.org/manual/Noweb-Reference-Syntax.html][noweb syntax]]. For example, the following block contains my name:
...
For example, to enable [[https://git-scm.com/book/en/v2/Appendix-A:-Git-in-Other-Environments-Git-in-Bash][Git prompt support in Bash]], I need to download a script from the Git repository and source it from =.bashrc=. The following block returns the full path where I want the file to live:
...
I’m sure there are more /Elisp-y/ ways to do this, but my goal was simply to show how Org Babel features, combined with a bit of Emacs Lisp, can make this setup a flexible system for managing configuration files.
